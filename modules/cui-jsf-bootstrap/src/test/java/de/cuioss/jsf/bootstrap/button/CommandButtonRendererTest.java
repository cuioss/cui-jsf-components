...
@DisplayName("Tests for CommandButtonRenderer")
class CommandButtonRendererTest extends AbstractComponentRendererTest<CommandButtonRenderer> {
    ...
    class ContentRenderingTests {
        ...
        void shouldHandleIconOnLeft(FacesContext facesContext) throws Exception {
            ...
            // Act & Assert
            var expected = new HtmlTreeBuilder().withNode(Node.BUTTON).withStyleClass(DEFAULT_BTN_CLASSES)
                    .withNode(ICONCOMPONENT).currentHierarchyUp().withNode(Node.SPAN)
        ...
        void shouldHandleIconOnRight(FacesContext facesContext) throws Exception {
            ...
            var expected = new HtmlTreeBuilder().withNode(Node.BUTTON).withStyleClass(DEFAULT_BTN_CLASSES)
                    .withNode(Node.SPAN).withStyleClass(CssBootstrap.BUTTON_TEXT).withTextContent(TEXT_VALUE)
                    .currentHierarchyUp().withNode(ICONCOMPONENT);
            assertRenderResult(component, expected.getDocument(), facesContext);
        }
