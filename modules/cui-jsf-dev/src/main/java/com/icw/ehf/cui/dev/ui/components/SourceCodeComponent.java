...
public class SourceCodeComponent extends BaseCuiNamingContainer {
    ...
    /** The component Id. */
    public static final String COMPONENT_ID = "com.icw.ehf.cui.dev.ui.components.SourceCode";

    /** Renderer type. */
    public static final String RENDERER_TYPE =
        "com.icw.ehf.cui.dev.ui.renderer.SourceCodeComponentRenderer";

    /** The component family. */
    public static final String COMPONENT_FAMILY_FIELD = "com.icw.ehf.cui.dev.ui";
    ...
    private String resolveFromSourcePath(final String sourcePath) {
        final URL resolved = resolvePath(sourcePath);
        if (null == resolved) {
    ...
    private String determineViewRelativePath(final String path) {
        final String requestPath = NavigationUtils.getCurrentView(getFacesContext()).getViewId();
        final String currentFolder = requestPath.substring(0, requestPath.lastIndexOf('/') + 1);
    ...
    private String resolveFromContainerId(final String sourceContainerId) {
        ...
        final FileLoader loader = FileLoaderUtility.getLoaderForPath(META_INF_PREFIX + viewId);
        Document document = null;
        try (final InputStream inputStream = loader.inputStream()) {
        ...
        final String query = String.format("//*[@id='%s']", sourceContainerId);
        ...
        final List<Element> filteredElements = xpath.evaluate(root);
    ...
    public Integer getMaxLineLength() {
        final Integer actual = state.get(MAX_LINE_LENGTH_ATTRIBUTE_KEY, LINE_LENGTH_DEFAULT);
    ...
    public String getType() {
        final String defaultValue = LangStyle.LANG_HTML.getStyle();
        String type = state.get(TYPE_ATTRIBUTE_KEY);
        if (null == type) {
            if (null == getSourcePath()) {
                return defaultValue;
            }
            final String path = getSourcePath().toLowerCase(Locale.ENGLISH);
            final String extension = path.substring(0, path.lastIndexOf(".") + 1);
            switch (extension) {
                case "properties":
